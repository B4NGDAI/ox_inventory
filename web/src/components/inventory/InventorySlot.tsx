    ...
    >
      <div
        ...
        style={{
          ...
          border: isOver ? '1px solid rgba(150,28,38,1)' : '',
        }}
      >
        {isSlotWithItem(item) && (
          ...
          background: (item?.durability == 0) ? 'radial-gradient(circle, rgba(136,0,0,0.6979166666666667) 0%, rgba(136,0,0,0.4990371148459384) 40%, rgba(255,255,255,0) 80%)' : undefined
            ...
            {inventory.type !== 'shop' && item.count && (
            <div className="item-slot-count-wrapper">
              <p style={{
                fontSize : item.count < 999 ? '18px': '12px'
              }}>{item.count ? item.count.toLocaleString('en-us') : ''}</p>
            </div>
            )}
            <div>
              {inventory.type !== 'shop' && item?.durability !== undefined && (
                <WeightBar percent={item?.durability ? (item?.durability ) : 0} durability /> //check this
              )}

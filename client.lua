...
function useSlot(slot)
	...
	if canUseItem(data.ammo and true) then
		...
		--if durability and durability <= 100 and consume then
			if durability and durability <= 0 and not data.weapon then
		...
		elseif data.weapon then
            ...
            -- end
			-- local hasWeapon, weaponHash = GetCurrentPedWeapon(playerPed, false, 0, false);
			-- if data.hash ~= weaponHash then
            --     --return lib.notify({ type = 'error', description = locale('cannot_use', data.label) })
            -- end
			...
			useItem(data, function(result)
				if result then
					currentWeapon = Weapon.Equip(item, result)
		...
		elseif currentWeapon then
			if data.ammo then -- if uses weapon ? as per items/shared.lua
				...
				-- end
				-- Using Native IsAmmoTypeValidForWeapon
				if not Citizen.InvokeNative(0xC570B881754DF609, currentWeapon.hash, joaat(data.name)) then
					return lib.notify({title = 'Ammo',description = 'Wrong Ammo',type = 'error'})
				end
				...
				useItem(data, function(resp)
					...
					if cache.vehicle then
						...
						if not (currentWeapon.metadata?.ammoType == `AMMO_ARROW`) then
							...
							Citizen.InvokeNative(0xB6CFEC32E3742779,playerPed, `AMMO_ARROW`, GetPedAmmoByType(playerPed, `AMMO_ARROW`), `REMOVE_REASON_DEBUG`)
						end
			...
			elseif data.allowArmed then -- Use Item when armed
				useItem(data)
			end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	client.interval = SetInterval(function()
		...
		--  local hasWeapon, weaponHash = GetCurrentPedWeapon(playerPed, false, 0, false);
		--  AntiCheat
		local weaponHash = Citizen.InvokeNative(0x8425C5F057012DAB, playerPed)
		...
		-- 			SetPedAmmo(playerPed, currentWeapon.hash, currentWeapon.metadata.ammo)
		-- 			SetPedCurrentWeaponVisible(playerPed, true, false, false, false)
		-- 			print('Set Wapon')
		-- 			--hasWeapon, weaponHash = GetCurrentPedWeapon(playerPed, false, 0, false);
		-- 			weaponHash = Citizen.InvokeNative(0x8425C5F057012DAB, playerPed)
		-- 		end
	...
	client.tick = SetInterval(function()
		...
		if not currentWeapon then 
			--DisableControlAction(0, 0x07CE1E61, false) -- Mouse Left Click
		end
	...
	-- 			DisableControlAction(0, 140, true)
				...
				elseif not currentWeapon.canFire then
	 			...
	 			if not invBusy and currentWeapon.timer ~= 0 and currentWeapon.timer < GetGameTimer() then
	 				...
	 				if weaponAmmo then
						...
						-- if currentWeapon and GetPedAmmoByType(playerPed, joaat(currentWeapon.metadata.ammoType)) == 0 then
						-- 	local slotId = Inventory.GetSlotIdWithItem(currentWeapon.metadata.ammoType, false)
						-- 	if slotId then
	 					-- 		useSlot(slotId)
	 					-- 	end
						-- end

	 				elseif currentWeapon.metadata.durability and currentWeapon.melee then
				...
				elseif currentWeapon.throwable then
	 			...
	 			elseif currentWeapon.melee and IsControlJustReleased(0, 0x07CE1E61) and IsPedInMeleeCombat(PlayerPedId()) then
	 				currentWeapon.melee += 1
	 				currentWeapon.timer = GetGameTimer() + 200
				elseif currentWeapon.fuel then	
					local durabilityDrain = Items[currentWeapon.name].durability
	 				currentWeapon.metadata.durability = (currentWeapon.metadata.durability - durabilityDrain < 0) and 0 or (currentWeapon.metadata.durability - durabilityDrain)
					if currentWeapon.metadata.durability <= 0 then 		
						currentWeapon = Weapon.Disarm(currentWeapon, nil, true)
					end
					invOpen = false
				end
	 		end

...
local function getItem(_, name)
    ...
    if name:sub(0, 7) == 'weapon_' then -- no need ?
        name = name:upper()
    end
...
CreateThread(function()
	...
	shared.info(('Inventory has loaded %d items in modules/items/server.lua'):format(count))

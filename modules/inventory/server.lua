...
-- Inventory.RemoveItem(inventory, ammo, diff, ammoType)
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal)
	...
	if count > 0 then
		...
		if not itemSlots then print('No Slots') return false end

		--------------------------------
		-------- BREAKS HERE------------
		--------------------------------
		if totalCount and count > totalCount then
			if not ignoreTotal then print('not_enough_items') return false, 'not_enough_items' end
			count = totalCount
		end
		...
		if weapon and weapon.metadata then
			...
			elseif action == 'ammo' then
				...
				elseif value < weapon.metadata.ammo then
					...
					if weapon.metadata.ammo == 0 then 
						weapon.metadata.ammo = nil
